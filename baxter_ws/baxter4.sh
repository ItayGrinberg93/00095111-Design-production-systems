...
baxter_hostname="baxter4.local"

# Set *Either* your computers ip address or hostname. Please note if using
# your_hostname that this must be resolvable to Baxter.
your_if=`ifconfig  | grep eth | head -1 | awk '{print $1}'`
your_ip=`ifconfig | awk -v ifnname=${your_if} '{if (substr($0,1,1) != " ") {ifn = $1;} if (($1 == "inet") && (substr($2,1,5) == "addr:") && (ifn == ifnname)) {print substr($2,6);}}'`
#your_ip="132.68.162.113"
